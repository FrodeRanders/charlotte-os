...
provides dynamically-sized blob storage on top of a block device. Objects
...
\subsection{Crash-safety design}

The intended update protocol writes new data extents first, then updates the
directory entry's \texttt{first\_extent\_lba} pointer with a single-block
write. This assumes that the device provides the required atomicity and
ordering after a flush. If a crash occurs between writing the extent and updating the
...
\begin{enumerate}
    ...
    \item \textbf{Crash-safety mechanism under development.} The pointer-swap
        design aims to preserve the previous version across an interrupted
        write. Power-loss testing and validation of device atomicity and flush
        ordering are still required.
