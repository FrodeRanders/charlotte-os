...
LBA 1:     reserved allocation metadata
LBA 2--33: object directory (512 entries × 32 bytes:
             id | flags | size_bytes | first_extent_lba)
LBA 34+:   variable-sized contiguous object extents
\end{verbatim}

\subsection{Current durability boundary}

The prototype writes replacement data to a newly selected extent, updates its
directory entry, and exposes \texttt{FLUSH}. Whole-object transfers use
multi-page memory objects and are divided into block requests of at most
2 MiB, so the NVMe PRP-list limit is not an object or file limit. The
directory's 32-bit byte length limits an individual object to 4 GiB and
available disk space. This is sufficient for the tested
...
commit protocol. Atomic recovery after sudden power loss is not yet
