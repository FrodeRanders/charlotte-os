...
next:      512-entry directory:
             id | flags | generation | header_lba |
             header_blocks | crc32
data:      object header followed by its data extent
\end{verbatim}
...
without moving the data offset contract; the current implementation has one
...
Directory records are checksummed but not journalled; torn-sector recovery and
full sudden-power-loss atomicity are not yet claimed. FNV-1a detects accidental
corruption, but is not cryptographic authentication.
