...
9 & Virtio-net driver: PCI discovery, MSI-X and SMMU delegation, feature
    ...
    Data path TCG-validated (relmsg/disco/dns/tcpip two-guest tests) \\
...
\begin{itemize}
    ...
    \item \textbf{Raft:} two-node election over local endpoint IPC on 4-LP
        ...
        terminate rather than polling. The distributed name service replicates a
        \texttt{name $\rightarrow$ node} catalog across two guests.
...
\begin{itemize}
    \item \textbf{HVF does not honor hardware ASIDs.} Apple's
        ...
        See \emph{AArch64 Port Status} for the full diagnosis.
    ...
    \item \textbf{Cross-machine Raft:} two TCG guests share a QEMU stream LAN;
