...
9 & Virtio-net driver: PCI discovery, MSI-X and SMMU delegation, feature
    ...
    Smoltcp 0.13 adapter + TCP/IP service binary compile. &
    Driver smoke path TCG-validated; full data path pending \\
...
\begin{itemize}
    ...
    \item \textbf{Virtio-net runtime:} initialization and transmit submission
        ...
        for direct EL0 device MMIO. Receive parsing, TX reclamation, batching,
        and the deployed TCP/IP service remain incomplete.
    ...
    \item \textbf{Cross-machine Raft:} two TCG guests can share a QEMU socket
        LAN, but reliable-message frame transport and remote Raft peer routing
    ...
    \item \textbf{Live upgrade:} the EL0 service manager performs handoff and
        invokes the capability-checked replacement-domain spawn path. It can
        ...
        Structural ELF validation and immutable kernel snapshotting are
        implemented. In the absence of signed packages, persistent images must
        exactly match the embedded build-authenticated image; signed package
        policy, rollback policy, multiple state objects, and manager-owned
