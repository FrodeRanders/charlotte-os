...
An endpoint answers one question: \textbf{``Who am I communicating with?''} It is
the mechanism by which protection domains invoke each other. This chapter explains
the endpoint IPC model --- CharlotteOS's primary communication primitive.
...
CharlotteOS supports two fundamentally different kinds of messages, and the ABI
keeps them distinct:
...
Scalar messages are \textbf{fast}: they require no memory-object allocation, no
page-table manipulation, and no TLB invalidation. The entire send/receive cycle
is a handful of register copies.
...
The server is never blocked waiting for one slow operation. The shard's executor
polls other tasks while the read completes.
...
A critical architectural distinction: endpoint IPC is for \textbf{cross-domain
...
\begin{enumerate}
    \item \textbf{Communication is authenticated by construction.} A client that
        ...
        is implicitly authenticated --- not by an IP address, not by a UID, but
        by possession of a kernel-mediated capability.
    ...
    \item \textbf{Scalar and memory messages are distinct.} The kernel does not
        ...
        messages stay small and fast. Bulk data moves as memory objects (Chapter~6)
        with explicit ownership semantics --- no accidental copies, no buffer
        over-reads.
    ...
    \item \textbf{Service restart is reliable.} Stale connections fail
        deterministically with typed errors (\texttt{EndpointClosed},
        \texttt{ServiceRestarted}). A restarted service cannot accidentally
        inherit outstanding reply tokens from a previous instance.
