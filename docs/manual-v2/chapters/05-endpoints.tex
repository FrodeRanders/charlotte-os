...
transfer modes (see \autoref{chap:memory-objects}). These are suitable for:
...
\begin{itemize}
    \item Created by the kernel when a call message is enqueued.
    \item Bound to the pending call: it completes exactly that
        caller's future.
    \item Consumable exactly once: a second reply attempt returns
        an error.
    \item Invalidated by cancellation: if the caller times out or
        aborts, the reply token becomes inert.
    \item Invalidated by endpoint teardown: if the server dies,
        all outstanding reply tokens are invalidated, and all waiting
        callers receive \texttt{ServiceRestarted}.
    \item Optionally delegable: a server may forward a reply token
        to another service, allowing a chain of services to contribute to
        a single reply.
\end{itemize}

\architecturefigure[0.93\linewidth]{capability-safe-ipc}
    {A service creates and registers its endpoint before a client resolves an
    attenuated connection and performs a capability-safe IPC call.}
    {fig:capability-safe-ipc}
...
\begin{itemize}
    \item Registration: a service registers as \texttt{"driver.nic.v1"}
        with a specific endpoint. The name service stores the endpoint's
        connection.
    \item Generation tracking: each registration bumps a generation
        counter. Stale client connections bound to the previous generation fail
        with \texttt{ServiceRestarted}.
    \item Access-key gating: a registration may specify a non-zero access
        key. The userspace name service, not the kernel, denies lookups without
        the matching key.
    \item Lookup: a client asks for \texttt{"driver.nic.v1"} and receives
...
\section{Authority, concurrency, and restart}

\begin{enumerate}
    \item Communication carries authority: a client that
    ...
    \item Scalar and memory messages are distinct: the kernel does not
        copy arbitrary data between address spaces by default. Small control
        messages stay small. Bulk data moves as memory objects
        (\autoref{chap:memory-objects}) with
        explicit ownership semantics.

    \item Deferred replies enable efficient concurrency: a server is never
    ...
    \item Restart errors are defined: stale connections return typed
    ...
    \item Name service is policy, not mechanism: the kernel provides
