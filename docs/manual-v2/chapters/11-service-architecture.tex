...
This chapter explains how protection domains are created, how services are named
and discovered, and how the system recovers from failures --- all without
privileged code knowing what any particular service \emph{does}.

\section{Protection domains as failure boundaries}

A protection domain (Chapter~3) is the unit of both authority and failure. When
a service crashes:

\begin{itemize}
    \item Its capability table is destroyed. Every connection to every client is
...
A crash in one domain does not propagate to the kernel or to other domains.
The failure is contained by the MMU and the capability system.

\section{The supervisor --- domain lifecycle manager}

The \textbf{supervisor} is a kernel-side component that creates and destroys
protection domains. It does not understand service protocols but it do understand
ELF images, memory mappings, and capability grants.

Its operations are:

\begin{enumerate}
    \item \textbf{Load:} parse an ELF image, validate its segments, create a
        new address space, map \texttt{PT\_LOAD} segments at their linked
        virtual addresses, set up a stack with a guard page.
    \item \textbf{Grant:} populate the domain's config-page with the bootstrap
        ...
        \end{itemize}
    \item \textbf{Spawn:} create the initial thread with the domain's entry
        point and its affinity LP.
    \item \textbf{Monitor:} register to be notified when the domain exits.
    \item \textbf{Teardown:} on exit, reclaim all capabilities, revoke
...
The name service runs as a \texttt{no\_std} EL0 service process, reached through
the bootstrap connection every domain receives at startup.
...
\begin{enumerate}
    \item \textbf{Device reset.} If the service was a driver, the supervisor
        resets the device before re-granting it to a new driver instance. The
        new driver sees a clean hardware state.
    \item \textbf{Generation increment.} The name service bumps the service's
        generation. All old connections are invalidated.
    \item \textbf{Stale connection failures.} Clients holding connections to
        the old instance receive \texttt{ServiceRestarted} on their next call
        attempt. They re-lookup the service and receive a connection to the
        new instance.
    \item \textbf{Outstanding operation reconciliation.} The supervisor
    ...
    \item \textbf{Fresh bootstrap.} The new domain receives a fresh config-page
...
A restarted service must not silently inherit outstanding authority or
in-flight operations from the previous instance. The kernel and the supervisor
guarantee this.
...
\begin{itemize}
    \item \textbf{The kernel} --- capabilities, endpoints, memory objects,
        CQs, scheduling, interrupt routing.
    \item \textbf{The supervisor} (kernel-side) --- domain lifecycle, ELF
        loading, device enumeration.
    \item \textbf{The name service} (userspace) --- service registration,
        lookup, access-key gating, generation tracking.
    \item \textbf{Service processes} --- one or more per logical function:
        NIC driver, TCP/IP stack, filesystem, Raft consensus, application
        servers.
    \item \textbf{Client processes} --- application code that looks up
...
\begin{enumerate}
    \item \textbf{Failure containment is architectural.} If the NIC driver
        crashes, the TCP/IP service loses connectivity but does not crash.
        If the filesystem crashes, applications holding file handles get
        errors. If the name service crashes, lookup fails but existing
        connections remain valid. No single service crash takes down the
        system.

    \item \textbf{Recovery is defined, not ad-hoc.} The restart sequence
    ...
    \item \textbf{Policy lives in userspace.} The kernel enforces mechanism
    ...
    \item \textbf{The bootstrap contract is explicit and minimal.} Every
