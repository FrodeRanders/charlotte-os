...
\begin{itemize}
    ...
    \item Service processes: one or more per logical function, such as a
        NIC driver, TCP/IP stack, UTC time service, filesystem, Raft consensus,
        or application server.
    \item Client processes: application code that looks up
        services and invokes them through capabilities.
\end{itemize}

\architecturewidefigure{service-composition}
    {Steady-state userspace composition around the name service, capability
    delegation, network stack, storage, and external-service connectors.}
    {fig:service-composition}
...
\section{Failure containment and recovery}
