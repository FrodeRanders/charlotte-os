...
\section{A mismatch at the operating-system boundary}

CharlotteOS starts from a specific mismatch. Async runtimes, least-privilege
services, and isolated components are often built above interfaces organized
around blocking calls, ambient namespaces, and machine-wide policy. This manual
calls the resulting adapters, duplicated concurrency models, and policy layers
the \textbf{retrofit tax}.

\section{Sources of the retrofit tax}
...
That translation can introduce:
\begin{itemize}
...
A process on a conventional OS commonly begins with access to broad filesystem
and network namespaces. Security policy then removes authority with mechanisms
such as chroot, seccomp, and namespaces. This approach requires the developer to
enumerate what the process must not reach. Programs that omit a restriction
retain more authority than they need, which increases the effect of a compromise.
...
Address-space isolation contains faults between drivers and applications, but it
also puts IPC on performance-critical paths. A design that cannot keep that path
cheap faces pressure to move services back into the kernel.
...
rules between address spaces. If authority is ambient until restricted, then the
...
\begin{enumerate}
    ...
    \item Capability-based, instead of ambient, authority: a process should be
