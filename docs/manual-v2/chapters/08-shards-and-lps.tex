...
\label{chap:shards-and-lps}

CharlotteOS separates the hardware execution context (logical processor, or LP)
from the userspace ownership domain (shard).
...
An LP is a \textbf{place}: threads are pinned to an LP at creation and, by default,
...
\begin{enumerate}
    \item Only the owning shard mutates shard-local state: there is no
        lock on shard-local data because there is no concurrent access. The
        executor runs one task at a time on the shard.

    \item Cross-shard interaction goes through bounded typed messages:
    ...
    \item Placement is explicit: code that needs to be shard-local is
        accessed through \rusttype{ShardLocal<T>}, which verifies at runtime that
        the calling thread belongs to the owning LP.

    \item Observability returns owned snapshots: diagnostics and
        introspection capture state by value. No borrowed reference into shard
        internals escapes the shard.
\end{enumerate}

\section[The usual mapping: one shard per LP]{The usual mapping: one shard $\leftrightarrow$ one LP}
...
\begin{tikzpicture}[
    ...
    \node[box] (shard) {one sitas shard};
    \node[box, right=0.8cm of shard] (thread) {one LP-affine\\userspace thread};
    \node[box, right=0.8cm of thread] (cpu) {normally one\\logical CPU};
    \draw[<->] (shard) -- (thread);
    \draw[<->] (thread) -- (cpu);
\end{tikzpicture}
\end{center}

The userspace thread and its executor are pinned to one LP.
\rusttype{ShardLocal<T>} state is valid only while that LP's owning thread runs.

The architecture retains the distinction (shard $\neq$ LP) so that more complex
configurations remain possible:

\begin{itemize}
    \item Multiple sharded processes: one LP might host shards from
        multiple protection domains, time-sliced by the kernel scheduler.
    \item CPU hotplug: an LP can be taken offline; its shards can be
        migrated (when migration-safe) or torn down.
    \item Oversubscription: a test can run $N$ shards on $M < N$ LPs
...
\section{Ownership and placement consequences}

\begin{enumerate}
    \item Single-owner mutation: the intended shard path does not access
        shard-local state concurrently. Runtime owner and borrow checks complement
        Rust's type rules.

    \item Placement is deliberate: sending work to a
    ...
    \item The two container types force a design choice: when you reach
        ...
        \rusttype{ShardLocal<T>} without locking. The choice is explicit.

    \item Cross-shard communication is typed and bounded: A
