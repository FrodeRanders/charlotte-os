...
userspace image from changing between validation and mapping. The test suite
installs \texttt{echo.elf} as a known NVMe object and verifies that a later
generation is loaded from that object.

This is structural validation, not package authentication. The current
prototype does not verify a signature, publisher identity, rollback version,
or measured-boot policy. Consequently, write authority to the executable
object store is update authority and must be delegated narrowly.
