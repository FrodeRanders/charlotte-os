...
The current suite contains twenty-two safe specifications. They are divided by
...
\tlbreak{CharlotteCluster}{Ingress} & Joint membership, replica placement,
    ...
    retained flow epochs & New flows use current policy; eligible backends are
...
The local name service now implements a co-located production authorization
path alongside its older public and reusable-key compatibility protocols. The
...
at redemption. Policy and audit state are not yet durable or replicated, and
the compatibility lookup opcodes must not be treated as this authorization
boundary.
...
for which all member routes are known. New flows require the latest locally
committed policy; established flows retain their selected policy while its
snapshot remains present. Safe history eviction removes dependent flow state
instead of silently choosing a new backend.

Three negative configurations expose the important cross-layer failures. One
admits a new SYN through a stale snapshot after current policy withdraws its
backend. Another evicts an epoch still named by a live flow and then remaps that
...
Snapshot freshness and unretained-flow handling remain implementation gaps:
the model records the stronger production contract instead of weakening the
invariant to match those gaps.
...
\tlbreak{UnsafeStartStale}{Flow} & A router admits a new SYN through an old
    snapshot after current policy withdraws its backend. &
    \tlbreak{NewFlowsUseLatest}{Policy} \\
\hline
\tlbreak{UnsafeFallbackExisting}{Packet} & Snapshot-history eviction makes an
...
The check suite expects all twenty-six negative configurations to fail for the named
...
\begin{itemize}
    \item runs all twenty-two safe configurations;
    ...
    \item runs twenty-six negative configurations and requires each one to fail on
...
\hline
\tlbreak{CharlotteCluster}{Ingress} & 672,087 & 151,904 & 14 \\
\hline
\end{longtable}
