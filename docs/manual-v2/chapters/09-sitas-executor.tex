...
The record set is generated on the fly; there are no files. The demo uses
512 records over two shard threads and three logical assembler work units.
A record's key is a pure function of its index (a SplitMix64 avalanche
over \texttt{seed} XOR \texttt{index}), so any thread can re-derive any key
without storing the data set. An index entry is a \texttt{(key, offset)}
pair.
