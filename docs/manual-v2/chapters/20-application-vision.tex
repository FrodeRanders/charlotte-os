...
operable after CI has built, signed, and uploaded the ELFs. Admission is still
per descriptor rather than an atomic signed process bundle. The remainder of
this chapter therefore separates the working vertical slice from the larger
...
submitted and observed together with \texttt{cluster-sign deployment-apply},
but its descriptors are independently committed and therefore do not yet form
an atomic release. Consuming a complete replicated Durga process bundle still
depends on signed bundle admission, the future placement controller, replica
routing, and production administration authority already identified there.
...
now be submitted through the RustFS-backed ingress and observed to readiness
with one host command, although it is not yet admitted atomically as a bundle.
The next experiment is to exercise that path with one transactional Kafka step
and then add signed, all-or-nothing bundle admission.
