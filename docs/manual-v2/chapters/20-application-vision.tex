...
named Kafka connector profiles, an explicit per-thread stack requirement, and
signed-deployment planning metadata. The
...
Stack deserves special treatment among those estimates. Only the application
and generator authors know the component's worst-case call depth, generated
adapter frames, and stack-resident data. The generated component plan should
therefore carry an explicit \texttt{stackPagesPerThread} request for developer
review. Operations may approve or reject it when signing the deployment, but
should not have to guess it from the ELF or silently rewrite it. Charlotte's
\texttt{CDEPLOY2} descriptor signs this value in 4 KiB pages; the kernel gives
every thread in the protected domain that exact limit. The current admissible
range is one through 64 pages, and legacy \texttt{CDEPLOY1} descriptors retain
the former four-page default.
