...
Each connector profile also fixes its own bounded registration name, so a
deployment can provision one independently named connector per authority set
without replacing a global \texttt{kafka} publication. The runner resolves
only the connector name declared by its step profile.
