...
defines the next boundary discussed later in this chapter.
...
language and runtime used to realise it.
...
files and preserves developer-owned business modules. Specialised services may
...
Code generation is important in both paths: Durga can also generate Kubernetes
manifests, container builds, and policy. Charlotte's proposed advantage is a
...
\begin{enumerate}
    \item stable request and response schema identities with serialisation code;
...
\begin{itemize}
    ...
    \item Generated capability plans and signed descriptor grants establish the
        request and admission shape. Inference rules, organisational review,
...
connector profiles, reviewed execution resources, signed replica policy, and
cluster deployment plans. Charlotte already admits those plans through signed
atomic releases and reconciles their replica sets. Semantic process bundles,
complete typed interfaces, and end-to-end provenance remain to be added.
...
Schedulers, durable logs, deployment policy, software supply chains, failure
recovery, and observability remain part of the system. Their expression moves
from containers and infrastructure manifests into a process compiler and a
cluster-native execution contract. The next falsifiable step is
