...
the domain and IOMMU state remain quarantined for diagnosis. The VirtIO entropy
adapter resets the device before dropping its shared-DMA mappings. The NVMe
...
NVM Flush completion, masks interrupts, clears 	exttt{CC.EN}, and waits for
	exttt{CSTS.RDY=0}. AHCI, VirtIO block, and NIC adapters still require their
controller-specific implementations. Thread death alone is never treated as
evidence that DMA stopped.
