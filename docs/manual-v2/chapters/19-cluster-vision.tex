...
shard-per-core model of Chapter~9 treats cores as executors over explicitly
owned state, while the scheduler in Chapter~10 moves work between them. The
...
\paragraph{Distributed-systems complexity is intrinsic.}
...
\paragraph{Compatibility and boundary-construction complexity may change.}
...
\begin{itemize}
    \item \textbf{Cross-machine consensus.} The Graft core replicates the
        ...
        \texttt{dns::OP\_CALL} (Chapter~14). The same replicated catalog
    ...
    \item \textbf{Reliable cluster messaging.} Messages up to 64~KiB are
        ...
        (Chapter~13). Remote name-service calls, deployment coordination,
    ...
    \item \textbf{Persistent per-node object stores.} The NVMe-backed object
        ...
        replacement (Chapter~15) for service artifacts and persistent Raft
    ...
    \item \textbf{Local stateful replacement.} Live service upgrade
        ...
        through the name service (Chapter~16). This has not yet been
        extended to mutable state crossing the network.

    \item \textbf{Service isolation and routing.} Services execute in
        separate address spaces with delegated capabilities (Chapter~11).
    ...
    \item \textbf{Certified work migration.} The scheduler migrates threads
        ...
        load rebalancing (Chapter~10). This is a per-node precedent for,
...
\begin{enumerate}
    \item \textbf{Discover the cluster.} The node uses broadcast discovery
        or a configured seed as discussed for Raft peers in Chapter~14.
...
(Chapter~9), and the distributed name service can identify which components
...
The distributed name service's Raft catalog (Chapter~14) also holds a
...
\begin{itemize}
    ...
    \item \textbf{Shared artifact availability.} Manifest records are
        ...
        NVMe volume. Network fetch, integrity-checked repair, retention,
        garbage collection, and replication policy remain.
