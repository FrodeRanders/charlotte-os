...
This chapter draws on three distinct kinds of evidence.
...
general-purpose, machine-centered operating systems to isolated, reproducible,
...
optimization from defeating availability. Node capacity, architecture,
...
A Linux container is not a new kernel object equivalent to a virtual machine.
It combines ordinary Linux processes with several existing facilities.
Namespaces determine which resources and identifiers are visible; cgroups
account for and limit resource use; Linux capabilities divide some root
privileges; seccomp restricts the system call interface; and an LSM such as
SELinux constrains object access. A prepared filesystem and network
configuration provide the process's apparent environment. An OCI image and
runtime turn those facilities into a
...
machine-centered operating system with broad compatibility obligations.
...
\begin{itemize}
    ...
    \item per-node application package management, configuration drift, and
        machine-centered deployment procedures; and
...
cluster executive may therefore look recognizably Kubernetes-like at the
...
Existing Linux software does not simply run on CharlotteOS. The system
exchanges compatibility for a smaller and
...
\begin{itemize}
    ...
    \item accepting authorized, generation-fenced desired-state changes;
...
must reject combinations the artifact did not authorize: in particular,
...
executable artifacts without an accepted public key. An authorized
...
\begin{enumerate}
    ...
    \item \textbf{Validation.} Each node validates every loaded ELF against
        ...
        not a general enrollment or rotation protocol.
...
leader resolves it to concrete assignments, and catalog-v15 snapshots retain
...
The scheduler remains intentionally bounded, but it now accounts for a useful
minimum of node capacity. Each node reports free and usable frames plus recent
CPU occupancy through the cluster's existing identity-checked peer path. The
leader rejects replayed epochs and low-pass filters free memory and CPU with an
integer EWMA before committing placement input through Raft. After three
missed reporting intervals it treats a sample as unknown. A new boot
incarnation resets the filter rather than blending unrelated processes.

This does not filter the observability archive. Raw one-second samples retain
short-lived spikes for diagnosis; the five-second report, EWMA, bucket
hysteresis, and freshness lease form a distinct control signal. The
descriptor's stack budget and fixed runtime pages form a memory promise. The
leader reconstructs promises held by other catalog deployments before
admitting a release, and every selected component debits projected dynamic and
static headroom. If a report expires while a node retains commitments, those
promises remain visible with no additional headroom instead of turning into an
apparently empty node. Hard memory admission preserves a one-sixteenth reserve,
while CPU pressure remains a soft ranking signal.

Pressure may now change an existing replica set, but only when the same
candidate survives a 30-second dwell; another pressure move is held for a
60-second cooldown. Membership loss and committed drain bypass dwell because
the old target is no longer eligible. The result is still committed as a
generation-fenced reassignment, so replacement readiness and DSR eligibility
advance together rather than following an uncommitted local judgment.

The policy does not yet express heap, processor, or storage requests, labels,
named failure domains, or more than one instance of an artifact on a node.
Reassignment advances the whole deployment generation, so retained replicas
restart rather than participating in a rolling surge/unavailability protocol.
...
nodes and use sustained behavior, not transient noise, to refine placement.

The node-capacity path is the first cluster-wide feedback loop, deliberately
limited to slow memory and CPU pressure. Detailed executor and communication
metrics are not yet aggregated across nodes or used for placement; extending
the controller requires similarly explicit filtering, dwell, authority bounds,
and decision telemetry rather than feeding raw measurements directly into an
actuator.

\subsection{A keyhole into the cluster}
\label{sec:cluster-keyhole}

The node HTTP keyhole remains useful for diagnosis, but placement and ingress
need an operator view whose subject is the cluster. The implemented
\texttt{/cluster} dashboard and \texttt{/cluster/metrics} JSON endpoint join
the Raft leader, term, commit index, and membership epoch with admitted and
draining nodes, filtered capacity freshness, desired and ready replicas, and
the advertiser and eligible backend set for each VIP. The schema is explicitly
versioned as \texttt{charlotte.cluster.v1}.

DNS constructs the underlying, bounded \texttt{CLOBSV1} record from one
locally applied catalog view. It serves the record only while the same
bounded-read lease used by DSR is valid; httpd decodes it and returns HTTP 503
when no fresh record is available. Raw one-second observations remain in the
node archive. The cluster view instead exposes the slower signals and
decisions that drive placement, while retaining a per-node freshness marker
so a committed resource promise is not confused with a recent capacity
measurement.

An unnamed operations VIP on port 80 can identify the built-in keyhole. The
leader normally advertises that VIP, but DSR may deliver the connection to any
admitted, non-draining member. Consequently each reply names both the leader
and the serving node. Leadership movement changes the advertiser without
making browser polling part of consensus or requiring the leader to terminate
every connection.

Membership presently supplies stable node identities and authenticated
Ethernet routes, not management IP addresses. The dashboard therefore does
not manufacture drill-down links from MAC addresses. A future signed, leased
management-endpoint record can support an authenticated
\texttt{/cluster/nodes/\{node-key\}/metrics} proxy, keeping node drill-down
behind the same policy as the cluster view.

The current port-80 adapter is unauthenticated and belongs only on a protected
operations network or test forwarding. Production use requires a separate
mTLS frontend with attenuated read capabilities. Its encrypted operational
profile should contain the server identity and observer-client trust anchor;
neither belongs in the application descriptor or Raft log. A browser then
uses an operations-issued PKCS\#12 client identity with \texttt{clientAuth},
not the cluster signing key. The complete boundary and browser setup are
documented in \repofile|docs/architecture/cluster-observability.md|.
...
\texttt{generation, signed descriptor\}}.
\end{quote}
...
The signed descriptor authorizes the deployment decision and binds artifact
...
exactly one signed descriptor as the body. The signature is the authorization
...
\architecturefigure{cooperative-deployment-shutdown}
...
every eligible voter, stable affinity/anti-affinity ranking, cumulative
memory-capacity admission, CPU-pressure ranking, rescheduling, and
...
needs explicit heap/CPU/storage requests, labels and named failure domains,
...
organizational key-management procedures.
...
authorization proof for the administrator who requested it.
...
placement, and DSR failover. Capacity-aware admission has unit and protocol
coverage; multi-node pressure and freshness scenarios still need a dedicated
fixture. Evidence is also required for rolling replacement, stateful cross-node
migration, artifact-store recovery, production key establishment and rotation,
and production administrative authorization.
...
\begin{itemize}
    \item \textbf{Dynamic membership recovery.} Discovery-bridged admission
        ...
        \texttt{FINALIZE} in the durable DNS-owned group. Partition behavior,
    ...
    \item \textbf{Placement and rollout depth.} The leader currently resolves
        fixed and every-node replica policy, basic affinity/anti-affinity,
        drain-aware membership, cumulative frame demand, and soft CPU pressure
        into assignments. The next scheduler adds typed heap/CPU/storage
        requests, trust labels, named failure domains, health and communication
    ...
    \item \textbf{Production administration authority.} The userspace
        ...
        authorization, audit, and safe concurrent administration remain.
...
\begin{itemize}
    ...
    \item \textbf{Plan 9} (Bell Labs) separates terminals, CPU servers, and
        file servers, and centralizes authentication and key handling in
...
\section{Research test}

The cluster vision can be summarized as the following proposition:
...
The next decisive evidence concerns richer resource declarations, stable
feedback-driven movement, rolling placement, state transfer, production trust
lifecycle, failure recovery, administration, and sustained operations.
Observing those mechanisms will show which complexity genuinely disappears and
which merely moves.
