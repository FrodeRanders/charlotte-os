...
What we can demonstrate now includes a three-node QEMU cluster with
discovery-bridged Raft admission, joint-consensus membership, and a cluster TCP
VIP whose connections are distributed without terminating TCP at the ingress
node. Replicated naming and deployment state, signed ELF artifacts, per-node
NVMe object stores, deployment agents, remote invocation, and a stateless
assignment handoff are also implemented. Automatic capacity-aware placement,
multi-replica placement, shared artifact storage, stateful cross-node
migration, and a production administration plane remain future work. For the
current singleton deployment model, cluster TCP ingress is already derived
from committed placement and exact-generation application readiness.
...
That indirection now has an external TCP counterpart. A node address remains
available when a protocol intentionally targets one machine, but a cluster
service VIP lets a client address functionality placed somewhere in Charlotte.
Direct Server Return distributes load, yet its deeper role here is to hide the
ingress and execution nodes behind a stable cluster network identity. The
...
\architecturewidefigure{two-node-cluster}
    {The demonstrated two-node cluster combines Raft-held compact desired state,
...
\begin{itemize}
    ...
    \item \texttt{notify}, which verifies a signed \texttt{CDEPLOY4}
...
This command is release orchestration rather than atomic process-bundle
admission. Each descriptor remains a separate Raft entry, so a later failure
can leave an accepted prefix committed. Repeating the same set is safe because
identical descriptors are idempotent.

The atomic alternative is a signed \texttt{CRELEASE} envelope produced with
...
\texttt{CDEPLOY4}; the reader retains compatibility with \texttt{CDEPLOY1}
through \texttt{CDEPLOY3}.
A follower relays the
request to the current leader, which resolves all automatic node selections
and proposes the set as one Raft command. The state machine preflights every
...
Admission atomicity does not make component startup simultaneous. Agents fetch,
launch, and publish independently after commit, and \texttt{release-apply}
waits for every exact generation. Coordinated rollback, progress policy, and a
semantic process bundle that also binds BPMN, schemas, provenance, and replica
intent remain controller work.
...
The deployment plane and cluster core are cooperative participants rather than
forced-retirement exceptions. Deployment ingress closes its listener, cluster
...
\texttt{committed}, \texttt{replacing}, or \texttt{ready}. A ready observation
is generation-safe: the active catalog owner must be the selected node and its
registration must carry the exact desired deployment generation. A stale
endpoint with the same name cannot make a replacement rollout appear ready.

The signed descriptor may carry node key zero to request automatic singleton
placement. The implemented initial policy chooses the current Raft leader,
independently of which member accepted the HTTP request; a nonzero key remains
an explicit pin. Full signed artifact names
...
administrator capability. The notification endpoint is a first off-cluster
management path, but it still needs network-level denial-of-service controls,
audit identity beyond the signing key, and production transport hardening.
...
Automatic placement currently covers one replica on the current Raft leader. A
cluster-level deployment planner is still required for capacity-aware node
selection, labels and failure domains, affinity and anti-affinity, replica
sets, rescheduling, readiness conditions, rollout surge/unavailability, and
rollback. Such a planner should commit desired assignments through Raft; node
agents should remain mechanism-only reconcilers.
...
\begin{itemize}
    ...
    \item \textbf{Durable artifact availability.} Signed descriptor deployment
        now pulls digest-pinned artifacts through a separately provisioned S3
        connector, while the legacy path still uses per-node NVMe staging.
        Integrity-checked repair, retention, garbage collection, connector
        bootstrap, and replication policy remain.
