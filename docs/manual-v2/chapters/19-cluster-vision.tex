...
The same mechanism now propagates a node drain through the deployment plane.
...
finally the object store. Each phase must be generation-safely reclaimed before
the next receives its request. Per-phase grace is capped by the enclosing node
deadline.

This coordinator deliberately stops before hardware-root teardown. It exposes
the frame router, NIC, block, and entropy domains exactly once to a subsequent
device-quiescence layer. Thread death is not evidence that DMA has stopped:
reset acknowledgement, interrupt masking, durable flush, DMA cessation, and
IOMMU invalidation must precede address-space reclamation.
