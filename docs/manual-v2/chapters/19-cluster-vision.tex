...
assignment handoff are also implemented. Signed fixed replica counts,
emph{every eligible node}, basic affinity/anti-affinity planning, membership
reconciliation, and per-node readiness now feed cluster TCP ingress. Automatic
capacity-aware placement, shared artifact storage, stateful cross-node
migration, rolling replacement, and a production administration plane remain
future work.
...
\begin{itemize}
    \item \textbf{Cross-machine consensus.} The Graft core replicates the
        ...
        Signed placement rules, concrete replica sets, and per-node readiness
        are stored in that state machine.
...
semantic process bundle that also binds BPMN, schemas, and provenance remain
controller work.
...
It does not prove resource-aware placement, rolling replacement, stateful
