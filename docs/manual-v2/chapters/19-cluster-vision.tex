...
CPU occupancy through the cluster's existing identity-checked peer path. Reports
carry an entropy-derived boot incarnation and are committed through Raft;
after three missed reporting intervals the leader treats a sample as unknown.
The descriptor's stack budget and fixed runtime pages form a memory demand.
Hard memory admission preserves a one-sixteenth reserve, while CPU pressure is
a soft ranking signal. Every selected component debits a projected capacity
view during one release decision, including explicit node pins, so several
individually valid components cannot overcommit the same sample.
...
The committed node-capacity reports are the first cluster-wide feedback loop,
deliberately limited to slow, bucketed memory and CPU pressure. Detailed
executor and communication metrics are not yet aggregated across nodes or used
to move existing assignments; those measurements are inputs from which a
stability-controlled placement controller can later be built.
