\chapter{Completion Queues --- Asynchronous Operations Without Loss}

A completion queue is the mechanism by which the kernel delivers results of
asynchronous operations to userspace. This chapter explains the CQ model, the
operation lifecycle, and the non-lossy invariant that makes CharlotteOS suitable
...
Use endpoint IPC (Chapter~5) when one protection domain invokes another.
...
The ring is mapped into the user address space with read-only permissions for the
consumer (userspace) and read-write for the producer (kernel). This means:

\begin{itemize}
    \item \textbf{Userspace drains entries without syscalls.} Advancing the
        consumer tail is a store to a shared page; reading entries is a load.
        No kernel crossing needed for the common fast path.
    \item \textbf{The kernel writes entries with a release fence.} The consumer
        sees coherent data without additional synchronization.
    \item \textbf{Overflow is detected, not hidden.} An overflow counter in the
...
\begin{itemize}
    \item \textbf{Retain overflow in a per-domain kernel backlog.} Entries that
        cannot fit in the ring wait in a kernel-side buffer. When userspace
        drains the ring, the backlog drains into it.
    \item \textbf{Stop accepting new submissions.} The kernel may return
        \texttt{WouldBlock} on \texttt{submit}, applying backpressure at the
        submission point rather than at the completion point.
    \item \textbf{Propagate \texttt{WouldBlock}.} The CQ's wait operation can
        indicate that the ring is full and must be drained.
\end{itemize}

The ring's overflow counter is \textbf{pressure telemetry} indicating to the consumer 
...
\begin{enumerate}
    \item \textbf{Timer events stay on the same LP's queue,} eliminating
    ...
    \item \textbf{Cache warmth is preserved.} The waking thread finds its data
        in the LP's local caches.
    \item \textbf{Shard-to-LP affinity is maintained.} The sitas shard that blocked
...
This contract ensures that a buffer is never freed while still reachable by the
kernel or a device. The safe Rust wrapper around an owned buffer consumes the
value on submission and returns it on completion; there is no path to
use-after-free even through cancellation.

\section{Why completion queues matter for critical software}

\begin{enumerate}
    \item \textbf{Non-lossy completions are a correctness guarantee.} A disk-write
        confirmation, a timer expiry, or a sensor reading is a fact and losing it
        is a bug. CharlotteOS guarantees delivery.

    \item \textbf{The single aggregated wait eliminates a class of race conditions.}
    ...
    \item \textbf{Per-shard CQ rings enable predictable latency.} No shard wakes
        another shard's executor to deliver a completion. Wakes are targeted,
        bounded, and proportional to the actual work.

    \item \textbf{The CQ ring is zero-syscall in the common case.} The fast path
    ...
    \item \textbf{Cancellation is defined, not best-effort.} Every cancellation
