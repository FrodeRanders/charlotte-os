...
replicates a \texttt{name $\rightarrow$ node} catalog across two guests, a
...
capability RPC is implemented (\texttt{dns::OP\_CALL}).
...
execution within that window. The replicated catalog does not yet bind the
target service generation, and remote object-capability delegation remains
future work.
