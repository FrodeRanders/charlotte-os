...
userspace virtio-net driver, a smoltcp adapter, a TCP/IP service, and a local
reliable-message prototype. The NIC/TCP path has compile-time coverage but has
not been runtime-validated on the present macOS/HVF development setup, and
remote capability RPC is not yet implemented.
...
TCP/IP is intended to be supported as a userspace compatibility service, not
the native programming model. A smoltcp adapter implements the \texttt{phy::Device}
...
The TCP/IP stack itself uses smoltcp 0.13. It is wrapped in a userspace service
that is intended to register as
