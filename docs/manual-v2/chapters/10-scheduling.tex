...
\begin{itemize}
    \item \textbf{Interrupt-masking spin \texttt{Mutex}} guards the frame
        ...
        cursor, and the global allocator. It saves the caller's
        interrupt-enable bit, masks maskable IRQs, spins, and restores the bit
        on unlock. The masking exists to prevent a preemption deadlock: these
        locks are taken from both preemptible kernel threads and synchronous
        EL0 exception paths, so a timer-preempted owner could otherwise be
        starved forever.
