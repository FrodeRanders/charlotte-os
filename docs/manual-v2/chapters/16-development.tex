...
\begin{itemize}
    ...
    \item \textbf{QEMU} for AArch64 (\texttt{qemu-system-aarch64}) or x86-64
        ...
        write-up. HVF builds rely on the
...
# Supply bootstrap cluster identities for exact deployment generations;
# repeat the option for independently placed services. A committed CINGPOL1
# operations policy supersedes this table at runtime:
./scripts/run-aarch64.sh release \
  --cluster-service orders=10.0.2.42:8080 \
  --cluster-service payments=10.0.2.43:8080

# An unnamed platform service, eligible on every admitted non-draining node:
./scripts/run-aarch64.sh release --cluster-service 10.0.2.44:80
...
focused two-guest smoltcp exchange. \texttt{--live-upgrade-test} runs the isolated
...
\architecturefigure{boot-and-testing}
    {Ordinary boot enables production networking and services; test switches
    add validators, while explicit negative switches remove runtime features.}
    {fig:boot-and-testing}
...
\begin{itemize}
    ...
    \item \repofiletext|crates/catten/src/self_test/el0_raft.rs|{\texttt{el0\_raft.rs}}: focused two-member Raft election over local IPC.
    ...
    \item \repofiletext|crates/catten/src/self_test/el0_uart.rs|{\texttt{el0\_uart.rs}}: userspace UART driver (delegated MMIO +
        IRQ, crash recovery).
    \item \repofiletext|crates/catten/src/self_test/el0_sitas.rs|{\texttt{el0\_sitas.rs}}: sitas executor integration covering sharded KV,
...
\begin{enumerate}
    ...
    \item \repodir|crates/catten/src/cpu/multiprocessor|: SMP facilities including IPI,
