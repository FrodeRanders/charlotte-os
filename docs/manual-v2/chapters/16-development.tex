...
\begin{itemize}
    ...
    \item \textbf{QEMU} for AArch64 (\texttt{qemu-system-aarch64}) or x86-64
        (\texttt{qemu-system-x86\_64}). On macOS, Apple Hypervisor Framework
        (HVF) acceleration is used automatically. On Linux, KVM is required
        for virtio-net runtime validation (HVF does not emulate EL0 MMIO
        instructions).
