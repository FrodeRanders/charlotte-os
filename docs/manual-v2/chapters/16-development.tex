...
\begin{itemize}
    ...
    \item \textbf{QEMU} for AArch64 (\texttt{qemu-system-aarch64}) or x86-64
        ...
        \emph{AArch64 Port Status} write-up --- so HVF builds rely on the
...
\begin{itemize}
    ...
    \item \texttt{el0\_uart.rs} --- userspace UART driver (delegated MMIO +
        IRQ, crash recovery).
    \item \texttt{el0\_sitas.rs} --- sitas executor integration: sharded KV,
        the mailbox index demo, and the thread-join probe (Section~9.8).
