...
x86-64 custom target & Four-LP ring-3 service, storage, IOMMU,
    ...
    and dynamic-membership paths pass under QEMU TCG; UEFI, VT-d, NVMe, and
    node-local services also pass under VMware Fusion \\
...
appliance unless \texttt{--replace} is supplied.

The supplied VM has no NIC. VMware does not expose the virtio-net device used
by the current network service, so discovery, DNS, smoltcp, HTTP, and
multi-node cluster operations remain QEMU-only until a VMXNET3 or E1000E
driver is added. The complete setup and qualification record is in
