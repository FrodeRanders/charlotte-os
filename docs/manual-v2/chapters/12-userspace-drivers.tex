...
all interrupt vectors, and all kernel data structures. A buffer overflow,
use-after-free error, or race in a driver can corrupt the kernel and crash the
system.
...
and returns. The driver then performs \textbf{direct EL0 MMIO}: load and store
instructions to the mapped virtual addresses, with no kernel involvement on the
fast path.
...
\begin{itemize}
    ...
    \item With IOMMU/SMMU: the DMA domain capability binds the
        ...
        prevents even a malicious driver from using that device to DMA outside
        its domain.
\end{itemize}
...
\begin{center}
\begin{tikzpicture}[
    layer/.style={draw, rounded corners, align=center, minimum width=4cm, inner sep=5pt},
]
    \node[layer] (app) {Application};
    \node[layer, below=0.7cm of app] (net) {Network service};
    \node[layer, below=0.7cm of net] (nic) {NIC driver};
    \node[layer, below=0.7cm of nic] (hw) {Hardware};

    \draw[->] (app) -- node[right, align=center] {socket endpoint} (net);
    \draw[->] (net) -- node[right, align=center] {packet endpoint} (nic);
    \draw[->] (nic) -- node[right, align=center] {MMIO / DMA / IRQ} (hw);
\end{tikzpicture}
\end{center}
...
\section{Isolation and device access}
...
\begin{enumerate}
    ...
    \item Restart follows a defined path: the supervisor handles
        ...
        The supervisor supplies the common lifecycle sequence instead of
        requiring each driver to invent restart coordination.
