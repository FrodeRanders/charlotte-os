...
The test suite covers local two-node election, single-voter term recovery from
NVMe, and cross-machine catalog replication and invocation through \texttt{dns}.
A separate persistent-storage boot test exercises discovery-led admission through
\texttt{JOIN}, joint consensus, and \texttt{FINALIZE}. The production DNS voter
set remains statically launched, and power-loss behavior and generic
\texttt{raft}-service client commands are not validated end to end.
...
\texttt{dns} service exposes client commands end to end
...
\begin{enumerate}
    \item \textbf{Consistency primitives can be exposed as services.} A replicated
