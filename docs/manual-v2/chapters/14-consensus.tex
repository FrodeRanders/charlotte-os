...
Validation covers several scales. Unit tests exercise quorum accounting with
up to five configured voters, while TLA+ safety models exhaustively explore
three-node elections and membership transitions within their bounds. QEMU
fixtures cover single-voter NVMe recovery, focused two-guest catalog and
deployment paths, and three-guest discovery, membership, and ingress failover.
These are validation topologies; the Raft membership representation and quorum
logic are generic.

Each operational node runs exactly one durable Raft member inside
\texttt{dns}; membership, names, deployments, keys, and cluster events share
that member's ordered log. Multi-guest admission observes it through discovery
and exercises \texttt{JOIN}, joint consensus, and \texttt{FINALIZE}. The separate
...
Once the cluster is running, committed peer identities resolve through the
name service. A joined node receives connection capabilities for the current
voter and learner set:

\begin{lstlisting}[style=rustCode]
for peer_name in committed_members {
    let peer = name_service.lookup(peer_name)?;
    transport.install(peer_name, peer)?;
}
\end{lstlisting}

The capability carries communication authority; an IP address alone does not.
...
future RDMA transport (high-performance cluster). The consensus core does not
change. This permits:
...
Locally, log payloads can transfer via Move memory objects: the kernel flips
page-table ownership instead of copying bytes. A follower that receives a Move
attachment gets the same physical pages the leader held, without an intermediate
buffer or copy.
...
Focused two-guest tests cover catalog and deployment exchanges, and the
three-guest ingress fixture covers discovery-driven admission, leader failover,
and continued cluster operation. None of these fixture sizes bounds membership.
...
A cluster administrator provisions a connection to a known cluster member as a
``cluster capability'' and delivers it to new nodes via the config page.
...
task. There is no need for a polling loop and no thread dedicated to sleeping.

\section{Replication consequences}
