...
The current suite contains twenty safe specifications. They are divided by
...
\tlbreak{Charlotte}{ThreadJoin} & Recyclable TIDs, spawn generations, delayed
    ...
    generation; a missing or recycled slot completes immediately. \\
\hline
...
The check suite expects all twenty negative configurations to fail for the named
...
\begin{itemize}
    \item runs all twenty safe configurations;
    ...
    \item runs twenty negative configurations and requires each one to fail on
...
\hline
\texttt{CharlotteThreadJoin} & 37 & 32 & 9 \\
\hline
