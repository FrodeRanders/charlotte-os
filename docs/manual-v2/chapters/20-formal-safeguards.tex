...
The models and their configurations live in \filepath{docs/tla}. The concise
model inventory is in \filepath{docs/tla/README.md}; the concrete-to-abstract
mapping is in \filepath{docs/tla/CONFORMANCE.md}. Those files are maintained
...
\begin{enumerate}
    ...
    \item \textbf{Map back to code.} \filepath{docs/tla/CONFORMANCE.md} records
...
The current suite contains seventeen safe specifications. They are divided by
...
The check suite expects all ten negative configurations to fail for the named
...
\begin{itemize}
    \item runs all seventeen safe configurations;
    ...
    \item runs ten negative configurations and requires each one to fail on
...
\begin{enumerate}
    \item Search \filepath{docs/tla/CONFORMANCE.md} for the changed Rust
