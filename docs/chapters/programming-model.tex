...
\paragraph{ShardLocal<T>} Lock-free per-LP container. Use for single-LP,
thread-only data. Access: \texttt{sl.with(|val| \{ *val = 42; \})}.

\paragraph{ShardMailbox<M>} Typed bounded per-LP queue. Cloneable
...
\paragraph{IPI closure} \texttt{try\_run\_on\_lp(target\_lp, || \{ ... \})}.
Boxes arbitrary work and delivers it to the target LP's IPI queue. Returns the
closure on full queue.

\paragraph{Completion API} \texttt{submit}, \texttt{submit\_worker},
...
\paragraph{CQ ring} \texttt{CompletionQueueRing} for zero-syscall completion
