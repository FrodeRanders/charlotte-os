...
\paragraph{Entry} The trampoline unmasks interrupts, calls the entry function,
...
\paragraph{Blocking} A thread blocks by calling \texttt{block\_thread(tid,
...
\paragraph{Waking} \texttt{Waker::notify(tid)} calls
...
\paragraph{Yielding} \texttt{yield\_lp()} sets the context-switch-pending flag
