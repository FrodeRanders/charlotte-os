...
\begin{itemize}
    \item[1.] \checkmark Two EL0 domains communicate through a bounded endpoint
        without ASID/LP.
    \item[2.] \checkmark Deferred async call while a shard continues other tasks.
    \item[3.] \checkmark Moved memory object --- sender locked out until return.
    \item[4.] \checkmark Lending enforced by mappings, including death cleanup.
    \item[5.] \checkmark Single \texttt{CQ\_WAIT} for endpoint + completions +
        device interrupts.
    \item[6.] \checkmark Terminal CQ results survive ring overflow (non-lossy
        backlog).
    \item[7.] \checkmark Coalesced remote shard wakes (idempotent scheduler fix).
    \item[8.] \checkmark Userspace UART driver with only delegated MMIO + IRQ.
    \item[9.] \checkmark Restart invalidates stale connections + device reset +
        op reconciliation.
