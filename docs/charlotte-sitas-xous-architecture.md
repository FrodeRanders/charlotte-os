...
manifest values or capabilities. Startup input is explicit rather than hidden in a
...
manifest values through `Context`; raw offsets are not the intended application API.

### Remaining work

Launch ABI v2.0 now uses the shared no-std `charlotte-launch` crate. It declares
...
no longer corrupt manifest values, capability records, or the launch header.
...
typed access to the named launch manifest, bootstrap authority, device grants,
per-shard completion queues, and live-upgrade handoff state.

The kernel writes, and crt0 validates, launch ABI v2.0 before `main` runs. Its
fixed-width shared header contains a magic value, major/minor version, header
size, config-page size, feature flags, bounded manifest and capability-vector
locations, and declared heap/input/default-CQ layouts. Initial authority is a
typed bounded capability vector. Manifest records carry named unsigned,
