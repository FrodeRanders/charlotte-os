...
\begin{itemize}
    \item \textbf{AArch64:} boots, all self-tests pass, and the full EL0 service
        suite runs through \texttt{crt0/cmain}. Boot starts one shared
        node-local name service and delegates connections to Raft peers, echo,
        the service manager, drivers, and clients; registering another name
        does not create another registry. The sitas \texttt{basic\_kv} binary
        runs with 2 shards on per-shard CQ rings and reports
        \texttt{total\_len == 3}.
