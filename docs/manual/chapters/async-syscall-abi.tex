...
\texttt{submit(op\_code, buffer) -> Result<CompletionCap, SubmitError>}.
...
\texttt{wait(cap) -> Result<(), CapError>}. Blocks the calling thread
...
\texttt{poll(cap) -> Result<Option<Completed>, CapError>}. Returns
...
\texttt{cancel(cap) -> CancelState}. Requests cancellation of an
...
\texttt{close(cap) -> Result<(), CapError>}. Frees a capability-table
...
The caller's address space is never a userspace parameter. On AArch64 the SVC
path captures the running thread's address-space id in \texttt{TrapFrame.asid};
all address-space-scoped operations use that kernel-derived identity. Register
\texttt{x0} is return-only for the typed wrappers; syscall arguments begin in
\texttt{x1}. This is a security boundary, not a convenience convention:
userspace must not be able to name another address space by passing an id.

\section{The exit-observer completion path}

The ABI's intended completion mechanism is declarative: \textbf{the worker
thread exiting IS the completion event}. \texttt{submit\_worker(worker\_entry,
result)} spawns a worker thread in the caller's address space and registers the returned
