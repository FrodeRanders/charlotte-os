...
| Address spaces | `memory_map_any`, kernel scratch windows, ASID-generation-keyed allocation state, mapping/teardown serialization | Placement mechanics changed, ownership did not. `CharlotteAddressSpace` remains the relevant generation-identity model; conformance maps the scratch allocator and its post-invalidation reuse rule explicitly. |
