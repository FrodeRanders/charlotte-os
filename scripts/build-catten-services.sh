...
SERVICES=(ns echo client uart cclient servicemgr raft nvme nvme_client objstore objstore_client fs net nclient relmsg rclient tcpip tcpclient httpd time s3 s3_smoke kafka kafka_smoke kafka_step kafka_step_proc kafka_step_input rng observe disco frouter dns agent clusterctl greet)
