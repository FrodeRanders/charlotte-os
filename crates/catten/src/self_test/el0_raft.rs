...
mod inner {
    ...
    pub(super) const NS_ELF: &[u8] = include_bytes!("ns.elf");
    pub(super) const RAFT_ELF: &[u8] = include_bytes!("raft.elf");
    ...
    pub(super) const NS_INTERFACE: u64 = packed_name(b"NAME");
    ...
    pub(super) fn spawn_raft_node(args: &[u32], ns_handle: &NameServiceHandle) -> ServiceDomain {
    ...
    pub(super) fn test_el0_raft() {
        crate::logln!("[raft] two-node leader election test");
        ...
        for _ in 0..5 {
            ...
            if let Some(r1_conn) = r1_reply.cap {
                ...
                crate::logln!("[raft] r1 state");
                if state1 == 3 {
                    crate::logln!("[raft] PASS leader");
                    return;
                }
            }
        }

        crate::logln!("[raft] no leader (race in peer discovery?)");
    }
}
...
pub fn test_el0_raft() {
    // Raft self-test requires AArch64 (service ELFs are AArch64 binaries).
}
