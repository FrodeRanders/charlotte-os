...
///     mov  x19, x0                               // save cap
///     str  w19, [x9, #4]                         // publish cap to worker
...
const COORD_CODE: &[u8] = &[
    ...
    0x21, 0x00, 0x00, 0xd4, 0xf3, 0x03, 0x00, 0xaa, 0x33, 0x05, 0x00, 0xb9, 0x00, 0x00, 0x80, 0xd2,
    0x01, 0x00, 0x82, 0xd2, 0x21, 0x00, 0xa0, 0xf2, 0x22, 0x00, 0x80, 0xd2, 0xe1, 0x00, 0x00, 0xd4,
    0x00, 0x00, 0x80, 0xd2, 0xe1, 0x03, 0x13, 0xaa, 0x81, 0x00, 0x00, 0xd4, 0x09, 0x00, 0x84, 0xd2,
    0x29, 0x00, 0xa0, 0xf2, 0x2a, 0x01, 0x40, 0xb9, 0xea, 0xff, 0xff, 0x34, 0x2b, 0x29, 0x40, 0xb9,
    0x09, 0x00, 0x86, 0xd2, 0x29, 0x00, 0xa0, 0xf2, 0xcc, 0x1b, 0x98, 0x52, 0x33, 0x05, 0x00, 0xb9,
    0x2b, 0x09, 0x00, 0xb9, 0xbf, 0x3b, 0x03, 0xd5, 0x2c, 0x01, 0x00, 0xb9, 0x08, 0x01, 0x00, 0xd4,
];

/// Worker stub. Loads the opaque capability published by the coordinator,
/// completes it with result 42, then exits.
///
/// ```asm
///     movz x9, #0x3000; movk x9, #0x1, lsl #16 // x9 = RESULT_VADDR
///     ldr  w1, [x9, #4]                        // opaque completion cap
///     mov  x0, #0; mov x2, #42; svc #2         // COMPLETE(cap, 42)
///     svc #8                                    // THREAD_EXIT
/// ```
#[cfg(target_arch = "aarch64")]
const WORKER_CODE: &[u8] = &[
    0x09, 0x00, 0x86, 0xd2, 0x29, 0x00, 0xa0, 0xf2, 0x21, 0x05, 0x40, 0xb9, 0x00, 0x00, 0x80, 0xd2,
...
extern "C" fn verify_el0_demo() {
    ...
    loop {
        ...
        if sentinel == 0xc0de {
            ...
            assert_ne!(cap, 0, "EL0 xLP: coordinator returned an invalid null capability");
            assert_eq!(
