...
///     mov  x0, #0; mov x1, #0; svc #1            // COMPLETION_SUBMIT -> x0 = cap
///     mov  x19, x0                               // save cap
///     mov  x0, #0; movz x1,#0x1000; movk x1,#0x1,lsl#16; mov x2,#1; svc #7  // SPAWN_THREAD worker@0x11000 on LP1
///     mov  x0, #0; mov x1, x19; svc #4           // COMPLETION_WAIT
///     movz x9, #0x2000; movk x9,#0x1,lsl#16      // x9 = CQ_VADDR
/// 1:  ldr  w10, [x9]; cbz w10, 1b                // poll ring head
///     ldr  w11, [x9, #40]                        // entry[0].result (offset 16+24=40)
///     movz x9, #0x3000; movk x9,#0x1,lsl#16      // x9 = RESULT_VADDR
///     movz w12, #0xc0de
...
#[cfg(target_arch = "aarch64")]
const COORD_CODE: &[u8] = &[
    0x09, 0x00, 0x86, 0xd2, 0x29, 0x00, 0xa0, 0xf2, 0x00, 0x00, 0x80, 0xd2, 0x01, 0x00, 0x80, 0xd2,
    0x21, 0x00, 0x00, 0xd4, 0xf3, 0x03, 0x00, 0xaa, 0x00, 0x00, 0x80, 0xd2, 0x01, 0x00, 0x82, 0xd2,
    0x21, 0x00, 0xa0, 0xf2, 0x22, 0x00, 0x80, 0xd2, 0xe1, 0x00, 0x00, 0xd4, 0x00, 0x00, 0x80, 0xd2,
    0xe1, 0x03, 0x13, 0xaa, 0x81, 0x00, 0x00, 0xd4, 0x09, 0x00, 0x84, 0xd2, 0x29, 0x00, 0xa0, 0xf2,
    0x2a, 0x01, 0x40, 0xb9, 0xea, 0xff, 0xff, 0x34, 0x2b, 0x29, 0x40, 0xb9, 0x09, 0x00, 0x86, 0xd2,
    0x29, 0x00, 0xa0, 0xf2, 0xcc, 0x1b, 0x98, 0x52, 0x33, 0x05, 0x00, 0xb9, 0x2b, 0x09, 0x00, 0xb9,
    0xbf, 0x3b, 0x03, 0xd5, 0x2c, 0x01, 0x00, 0xb9, 0x1f, 0x20, 0x03, 0xd5, 0xff, 0xff, 0xff, 0x17,
];

/// Worker stub. Completes capability 0 (the coordinator's first and only
/// submit) with result 42, then spins.
///
/// ```asm
///     mov  x0, #0; mov x1, #0; mov x2, #42; svc #2 // COMPLETE(cap=0, 42)
/// 1:  nop; b 1b
/// ```
#[cfg(target_arch = "aarch64")]
const WORKER_CODE: &[u8] = &[
    0x00, 0x00, 0x80, 0xd2, 0x01, 0x00, 0x80, 0xd2, 0x42, 0x05, 0x80, 0xd2, 0x41, 0x00, 0x00, 0xd4,
    0x1f, 0x20, 0x03, 0xd5, 0xff, 0xff, 0xff, 0x17,
];
