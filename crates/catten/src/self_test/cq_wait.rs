...
extern "C" fn cq_waiter() {
    ...
    completion::wait_on_cq(CQW_ASID, 1);
    assert!(
    ...
    completion::wait_on_cq(CQW_ASID, 1);
    assert_eq!(
        completion::cq_pending(CQW_ASID),
        0,
...
extern "C" fn cq_driver() {
    ...
    let operation = completion::submit_detached(CQW_ASID, OpCode::Nop, 0xd1)
    ...
    let ring_ptr = completion::cq_ring_of(CQW_ASID).expect("[cq wait] CQ ring missing");
    while unsafe { &mut *ring_ptr }.read().is_some() {}
    assert_eq!(completion::cq_pending(CQW_ASID), 0);

    ROUND2_START.store(1, Ordering::Release);
    for _ in 0..64 {
        yield_lp();
    }
    completion::wake(CQW_ASID);
    spin_until(&ROUND2_RELEASED, "wake release");

    logln!("[cq wait] SUCCESS: blocking CQ wait released by completion and by explicit wake.");
    loop {
        yield_lp();
    }
}
