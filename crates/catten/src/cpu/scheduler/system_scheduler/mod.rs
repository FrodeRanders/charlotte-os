...
impl SystemScheduler {
    ...
    /// Move one queued, non-pinned thread when an LP is idle and another LP
    /// has more than one runnable thread. Only `Ready` threads migrate: a
    /// running thread keeps its execution context local, while a blocked
    /// thread keeps the LP that owns any pending timer wake until it becomes
    /// runnable. The two scheduler locks are acquired in LP order and the
    /// thread state is revalidated under the table lock before mutation.
    pub fn try_rebalance(&self) {
        let loads: Vec<(LpId, usize)> = self
            .lp_schedulers
            .iter()
            .map(|(&lp, scheduler)| (lp, scheduler.lock().thread_count()))
            .collect();
        let Some(idle_lp) = loads.iter().find(|(_, load)| *load == 0).map(|(lp, _)| *lp)
        else {
            return;
        };
        let Some((busy_lp, _)) = loads.iter().filter(|(_, load)| *load > 1).max_by_key(|x| x.1)
        else {
            return;
        };
        let busy_lp = *busy_lp;
        let candidate = {
            let table = MASTER_THREAD_TABLE.read();
            table.iter().enumerate().find_map(|(tid, entry)| {
                let thread = entry.as_ref()?;
                (thread.pinned_lp.is_none()
                    && matches!(thread.state, ThreadState::Ready(lp) if lp == busy_lp))
        ...
        let still_migratable = MASTER_THREAD_TABLE.read().get(tid).is_ok_and(|thread| {
        ...
        let (source, destination) = if busy_lp == first_lp {
            (&mut first, &mut second)
        } else {
            (&mut second, &mut first)
        };
        if source.remove_thread(tid).is_err() {
            return;
        }
        {
            let mut table = MASTER_THREAD_TABLE.write();
            let thread = table.get_mut(tid).expect("rebalance candidate disappeared");
            thread.state = ThreadState::NeedsLpAssignment;
            thread.affinity_lp = Some(idle_lp);
        }
        destination
            .add_thread(tid, Some(generation))
            .expect("failed to admit rebalanced thread");
        destination.set_ctx_switch_pending();
        drop(first);
        drop(second);
        if idle_lp != get_lp_id() {
            LocalIntCtlr::send_unicast_ipi(idle_lp, SCHEDULER_IPI_VECTOR)
                .expect("failed to wake rebalanced LP");
        }
        sched_trace!("[sched] rebalance TID={} LP{} -> LP{}", tid, busy_lp, idle_lp);
    }
