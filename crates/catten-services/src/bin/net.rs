...
unsafe fn zero_vq(page_va: usize, queue_size: u16) {
    ...
    for i in 0..size {
        ...
        unsafe {
            w32(page_va + off + virtio::DESC_ADDR_LO, (page_va + 0x2000/* RX_BUF */) as u32);
        }
        unsafe {
            w32(page_va + off + virtio::DESC_LENGTH, 2048);
        }
        unsafe {
            w16(page_va + off + virtio::DESC_FLAGS, virtio::VRING_DESC_F_WRITE);
        }
...
fn main(ctx: Context) -> ! {
    ...
    if unsafe { r8(bar0 + virtio::DEVICE_STATUS) & virtio::STATUS_FEATURES_OK == 0 } {
        unsafe {
            w8(bar0 + virtio::DEVICE_STATUS, virtio::STATUS_ACKNOWLEDGE | virtio::STATUS_DRIVER)
        };
    }
    ...
    if _rx_cap == 0 || _tx_cap == 0 || rx_buf_cap == 0 {
        unsafe { thread_exit() };
    }
    ...
    for i in 0..qsz as usize {
        unsafe {
            w32(
                ...
                rx_buf_phys as u32 + (i * 2048) as u32,
            )
        };
    }
